...
class ClassHasProductController extends Controller
{
    public function render(Request $request)
    {
        ...
        return DataTables($classes)
            ->addColumn('products', function ($q) {
                $products = null;
                foreach (
                    \App\Models\Inventory\Product\ClassHasProduct::where('class_id', $q->class_id)
                        ->select('product_id', 'class_has_product_price')
                        ->get()
                    as $chp
                ) {
                    $products .=
                        '<div class="rounded-pill bg-success pl-2 pr-2" style="display: inline; padding:2px;">' .
                        $chp->product_id .
                        ' | ' .
                        \App\Models\Inventory\Product\Product::where('product_id', $chp->product_id)
                            ->select('product_name')
                            ->get()[0]->product_name .
                        ' (₹' .
    ...
    public function classHasProducts($id, Request $request)
    {
        ...
        $class_has_products = \App\Models\Inventory\Product\ClassHasProduct::leftJoin('products as p', 'class_has_products.product_id', 'p.product_id')
            ->leftJoin('product_categories as pc', 'p.product_category_id', 'pc.product_category_id')
            ->where('class_has_products.class_id', $id)
    ...
    public function create($id)
    {
        ...
        $products = \App\Models\Inventory\Product\Product::whereNotIn('product_id', function ($q) use($id) {
            $q->select('class_id')->from('class_has_products')->where('class_id', '!=', $id)->get();
        })
    ...
    public function store($id, Request $request)
    {
        ...
        if (
            \App\Models\Inventory\Product\ClassHasProduct::where('class_id', $id)
